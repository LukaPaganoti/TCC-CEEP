...
<body>
    ...
    <form method="post" class="form-group" enctype="multipart/form-data">
        ...
        <!-- Exibição de informações do endereço do cliente logado -->
        <label>Endereço de Entrega</label>
<select name="codendereco">
    <?php
    // Consulta SQL para obter os endereços cadastrados do cliente logado
    ...
    $stmt_enderecos_cliente->bindParam(':fk_codcliente', $codcliente);
    $stmt_enderecos_cliente->execute();
    $enderecos_cliente = $stmt_enderecos_cliente->fetchAll(PDO::FETCH_ASSOC);

    if ($enderecos_cliente) {
        foreach ($enderecos_cliente as $endereco) {
            echo "<option value='{$endereco['codendereco']}'>{$endereco['cepend']}, {$endereco['numeroend']} - {$endereco['bairroend']} - {$endereco['ruaend']} - {$endereco['complementoend']} - {$endereco['fk_codcidade']}</option>";
        }
    } else {
        echo "<option value='' disabled>Nenhum endereço cadastrado.</option>";
    }
    ?>
</select>
        <br><br>

        <!-- Link para redirecionar para a página de cadastro de endereço -->
        <a href="cadastroendereco.php">Cadastrar novo endereço</a>
        <br><br>
